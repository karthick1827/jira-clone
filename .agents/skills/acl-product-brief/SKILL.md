...
2. Finalize review status: Ensure `brief.md` frontmatter specifies `project_type: greenfield` (or `brownfield` if created during Brownfield Tier 2) and has `status: In Review` so it enters the official Markdown Studio approval queue for Manager sign-off.
